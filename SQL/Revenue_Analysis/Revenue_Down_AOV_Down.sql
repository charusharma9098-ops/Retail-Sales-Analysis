...
*/
 ...
 ROUND(SUM(Sales), 2)/COUNT(DISTINCT `Order ID`) AS total_AOV
 FROM superstore 
 ...
 /*
Category-level drill-down for AOV Down:
Compare category-level AOV between the selected months
to identify the category with the largest AOV decline.
*/
 ...
 /*
Product-level drill-down for AOV Down:
Compare product-level AOV between the selected months
to identify products contributing most to the AOV decline.
*/
...
/*
Region-level drill-down for AOV Down:
Compare region-level AOV between the selected months
to identify regions contributing most to the AOV decline.
*/
 ...
 AND `Order Date` < '2024-06-01'
 AND category="Technology"
 AND `Product Name`="External SSD 1TB"
 GROUP BY 
 YEAR(`Order Date`), 
 MONTH(`Order Date`), 
 MONTHNAME(`Order Date`),
